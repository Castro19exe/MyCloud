...
def home():
    ...
    if current_user.gcs_bucket_name:
        try:
            ...
            for blob in blobs:
                updated_date = blob.updated.strftime('%Y-%m-%d %H:%M') if blob.updated else "N/A"
...
def logout_page():
    logout_user() # Função do Flask-Login para terminar a sessão
...
def register_page():
    ...
    if request.method == 'POST':
        ...
        # Agora, tenta criar o bucket GCS
        try:
            # FORNEÇA O ID DO SEU PROJETO AQUI
            project_id = 'airy-bit-460116-k8' # <--- CONFIRME SE ESTE É O SEU ID DE PROJETO CORRETO
            storage_client = storage.Client(project=project_id)
            ...
            return jsonify({"message": "Registo bem-sucedido! Bucket criado. Por favor, faça login.", "redirect": url_for('login_page')}), 201
...
# ROTA DE RESET TOTAL - USAR COM EXTREMO CUIDADO!
